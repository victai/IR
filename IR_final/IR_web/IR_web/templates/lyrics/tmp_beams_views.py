...
cmd = ['t2t-decoder',
       ...
       '--return_beams=True',
       '--decode_hparams=\"return_beams=True\"']
...
def generate_sentence(input_sentence):
    ...
    output = re.findall(r'INFO.*SOS.*\n', child.before)
    print('output:', output)
    sentence_generated = []
    if output:
        for out in output:
            sentence = out.split(':')[-1]
            sentence_generated.append(sentence.replace('SOS', '').replace('EOS', '').replace('\"', '').strip())
...
def gen_model_input(rhyme, first_sentence, keywords, hid_sentence, length, pattern, selected_index):
    ...
    for row_num, length_row in enumerate(length):
        ...
        input_sentence = ''.join(input_sentence.split('SOS')[1].split('EOS')[0].split())
        ngram = 2 #len(input_sentence)
        match = [0] * len(sentence_now)
        while ngram > 0:
            start = 0
            while start+ngram <= len(input_sentence):
                word = input_sentence[start: start+ngram]
                for i, now in enumerate(sentence_now):
                    match[i] += len(re.findall(word, now))
                start += 1
            ngram -= 1
            if min(match) != max(match):
                break
        
        sentence_now = sentence_now[match.index(min(match))]
